...
export default function Header({ user }: HeaderProps) {
  ...
  function handleSearch(e: FormEvent<HTMLFormElement>) {
    ...
    const q = search.trim();
    if (q) {
      router.push(`/games?search=${encodeURIComponent(q)}`);
    } else {
      router.push('/games');
    }
  }

  // Safely fallback to /login if user or user.id is missing
    ...
    <header className="border-b border-white/5 bg-brand-bg sticky top-0 z-50 px-6 py-4">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <form onSubmit={handleSearch} className="relative hidden sm:block w-64">
            <input
              type="text"
              value={search}
            ...
            <button 
              type="submit" 
              className="absolute left-2.5 top-2 text-gray-600 hover:text-brand-primary-button transition cursor-pointer"
            >
              <svg 
